...
<style>
    ...
    .stButton>button { background-color: #1E3A8A; color: white; font-weight: bold; border-radius: 6px; width: 100%; }
</style>
...
with st.form("kpi_form"):
    ...
    with c_right:
        ...
        st.success(f"📊 準時率：**{rate_1_1:.1f}%** | 💡 即時得分：**{score_1_1} / 8** 分")
    ...
    with c_right:
        ...
        st.success(f"📊 合規率：**{rate_1_2:.1f}%** | 💡 即時得分：**{score_1_2} / 8** 分")
    ...
    with c_right:
        ...
        st.success(f"💡 即時得分：**{score_1_3} / 4** 分")
    ...
    with c_right:
        ...
        st.success(f"📊 完成率：**{rate_2_1:.1f}%** | 💡 即時得分：**{score_2_1} / 15** 分")
    ...
    with c_right:
        ...
        st.success(f"📊 通過率：**{rate_2_2:.1f}%** | 💡 即時得分：**{score_2_2} / 12** 分")
    ...
    with c_right:
        ...
        st.success(f"💡 即時得分：**{score_2_3} / 8** 分")
    ...
    with c_right:
        ...
        st.success(f"📊 準確率：**{rate_3_1:.1f}%** | 💡 即時得分：**{score_3_1} / 8** 分")
    ...
    with c_right:
        ...
        st.success(f"📊 損耗率：**{rate_3_2:.2f}%** | 💡 即時得分：**{score_3_2} / 7** 分")
    ...
    with c_right:
        ...
        st.success(f"📊 完成率：**{rate_4_1:.1f}%** | 💡 即時得分：**{score_4_1} / 12** 分")
    ...
    with c_right:
        ...
        st.success(f"📊 合規率：**{rate_4_2:.1f}%** | 💡 即時得分：**{score_4_2} / 10** 分")
    ...
    with c_right:
        ...
        st.success(f"💡 即時得分：**{score_4_3} / 8** 分")
    ...
    st.info(f"💡 **建議發放及跟進安排：** {grade_desc}")
    ...
    submitted = st.form_submit_button("💾 生成考核報告並導出 PDF", type="primary")
...
if submitted:
    if not emp_name:
        ...
        st.success("✅ KPI 評核報告已成功生成 (ReportLab)！")
